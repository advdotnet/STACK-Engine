...
namespace STACK
{
	public partial class Window : Game
	{
		...
		public void HandleDebugInputEvent(InputEvent input)
		{
			...
			{
				_bloomIndex = (_bloomIndex + 1) % BloomSettings.PresetSettings.Length;

				StackEngine.Game.World.Get<RenderSettings>().BloomSettings = BloomSettings.PresetSettings[_bloomIndex];
